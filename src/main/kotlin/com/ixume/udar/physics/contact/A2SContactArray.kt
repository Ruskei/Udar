...
/*
Single-threaded contact storage for Active-to-Static contacts
...
class A2SContactArray {
    ...
    fun add(
        ...
        val idx = cursor * A2S_CONTACT_DATA_SIZE
        cursor++
    ...
    fun transferToBuffer(contactIdx: Int, buffer: A2SContactBuffer, activeBody: ActiveBody) {
        ...
        buffer.add(
            ...
            pointAX = arr[baseIdx + POINT_A_X_OFFSET],
            pointAY = arr[baseIdx + POINT_A_Y_OFFSET],
            pointAZ = arr[baseIdx + POINT_A_Z_OFFSET],
            normX = arr[baseIdx + NORM_X_OFFSET],
            normY = arr[baseIdx + NORM_Y_OFFSET],
            normZ = arr[baseIdx + NORM_Z_OFFSET],
            t1X = arr[baseIdx + T1_X_OFFSET],
            t1Y = arr[baseIdx + T1_Y_OFFSET],
            t1Z = arr[baseIdx + T1_Z_OFFSET],
            t2X = arr[baseIdx + T2_X_OFFSET],
            t2Y = arr[baseIdx + T2_Y_OFFSET],
            t2Z = arr[baseIdx + T2_Z_OFFSET],
            depth = arr[baseIdx + DEPTH_OFFSET],
            normalLambda = arr[baseIdx + NORMAL_LAMBDA_OFFSET],
